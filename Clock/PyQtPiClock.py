...
def tick():
    ...
    if now.day != lastday:
        ...
        if Config.DateLocale != "":
            sup = ""
        ds = "{0:%A %B} {0.day}<sup>{1}</sup>".format(now, sup)
        ds2 = "{0:%a %b} {0.day}<sup>{1}</sup>".format(now, sup)
        datex.setText(ds)
        datex2.setText(ds2)
...
if not Config.digital:
    ...
    clockface.setStyleSheet(
        "#clockface { background-color: transparent; font-family:arial;" +
...
datex.setGeometry(0, 0, width, 150)
